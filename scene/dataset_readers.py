...
def readColmapSceneInfo(path, feature_type, images, eval, images_to_read=None):
    ...
    if '7scenes' in path or '12scenes' in path:
        sfm_path = os.path.join(path, "sfm_gt")
    elif 'cambridge' in path:
        sfm_path = os.path.join(path, "sparse", "0")
    else:
        raise ValueError(f"Unknown dataset: {path}")
    
    try:
        cameras_extrinsic_file = os.path.join(sfm_path, "images.bin")
        cameras_intrinsic_file = os.path.join(sfm_path, "cameras.bin")
        cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)
        cam_intrinsics = read_intrinsics_binary(cameras_intrinsic_file)
    except:
        cameras_extrinsic_file = os.path.join(sfm_path, "images.txt")
        cameras_intrinsic_file = os.path.join(sfm_path, "cameras.txt")
    ...
    if os.path.exists(os.path.join(path, "sfm_gt", "list_test.txt")):
        # 7scenes or 12scenes
        with open(os.path.join(path, "sfm_gt", "list_test.txt")) as f:
    ...
    ply_path = os.path.join(sfm_path, "points3D.ply")
    bin_path = os.path.join(sfm_path, "points3D.bin")
    txt_path = os.path.join(sfm_path, "points3D.txt")
