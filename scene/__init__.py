...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], num=-1, images_to_read=None, preload_cameras=True): 
        ...
        if preload_cameras:
            for resolution_scale in resolution_scales:
                if num != -1:
                    ...
                    self.test_cameras[resolution_scale] = cameraList_from_camInfos(scene_info.test_cameras[::step_test], resolution_scale, args)  if  args.eval else [] #####
                else:
                    print("Loading Train Cameras")
                    ...
                    print("Loading Test Cameras")
                    self.test_cameras[resolution_scale] = cameraList_from_camInfos(scene_info.test_cameras, resolution_scale, args)  if  args.eval else []  ########
        ...
        if self.loaded_iter:
            ...
            self.gaussians.create_from_pcd(scene_info.point_cloud, self.cameras_extent, args.speedup) 
    ...
    def getTrainCameras(self, scale=1.0):
        if self.preload_cameras:
            ...
            return DataLoader(SceneDataset(self, split='train'), batch_size=None, shuffle=self.shuffle, pin_memory=True, num_workers=0)

    def getTestCameras(self, scale=1.0):
        if self.preload_cameras:
            return self.test_cameras[scale]
        else:
            return DataLoader(SceneDataset(self, split='test'), batch_size=None, shuffle=self.shuffle, pin_memory=True, num_workers=0)
